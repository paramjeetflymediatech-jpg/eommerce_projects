        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 32 }}>
          ...
          <div className="card" style={{ padding: 32, textAlign: "center", height: "fit-content" }}>
            ...
            <div className={`badge ${user?.role === "ADMIN" ? "badge-primary" : "badge-secondary"}`} style={{ display: "inline-block", padding: "6px 16px" }}>
              {user?.role?.charAt(0).toUpperCase() + user?.role?.slice(1).toLowerCase() || "Customer"}
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
            <div className="card" style={{ padding: 32 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
                <div><label style={{ display: "block", fontSize: "0.85rem", fontWeight: 600, color: "var(--text-muted)", letterSpacing: "normal", marginBottom: 8 }}>Full Name</label><p style={{ fontWeight: 600 }}>{user?.name || "—"}</p></div>
                <div><label style={{ display: "block", fontSize: "0.85rem", fontWeight: 600, color: "var(--text-muted)", letterSpacing: "normal", marginBottom: 8 }}>Email Address</label><p style={{ fontWeight: 600 }}>{user?.email}</p></div>
                <div><label style={{ display: "block", fontSize: "0.85rem", fontWeight: 600, color: "var(--text-muted)", letterSpacing: "normal", marginBottom: 8 }}>Account Status</label><p style={{ color: "var(--success)", fontWeight: 600 }}>Active</p></div>
                <div><label style={{ display: "block", fontSize: "0.85rem", fontWeight: 600, color: "var(--text-muted)", letterSpacing: "normal", marginBottom: 8 }}>Member Since</label><p style={{ fontWeight: 600 }}>March 2026</p></div>
              </div>
