...
export default function EditBlogPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (blog) {
          ...
          setMsg({ text: "Blog not found.", type: "error" });
        }
      }
    } catch (e) {
      setMsg({ text: "Failed to load blog.", type: "error" });
    }
    setLoading(false);
  };
  ...
  if (status === "loading" || loading) return <div style={s.center}>Loading blog...</div>;

  return (
    <div style={s.mainContainer}>
      <div style={s.actionHeader}>
        <h2 style={{ margin: 0, fontSize: "1.5rem" }}>Edit Blog</h2>
